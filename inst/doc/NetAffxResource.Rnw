...
\Rfunarg{user}, \Rfunarg{password}, and \Rfunarg{affxLicence}, length
1 character vectors containing the registered user name and password,
and institutional licence (available from Affymetrix). The password is
